			...
			<view class="page cu-list menu card-menu margin-top" style="margin: 20rpx 20rpx;">
				...
				<radio-group class=" block margin-top-sm radius" @change="RadioChange">
					...
					<view class="cu-form-group"   v-if="payitem.payType=='0'"  >
						...
						<radio class="mradio blue" :class="payitem.isDefault?'checked':''" :checked="payitem.isDefault" :value="payitem.payType+''"></radio>
					</view>
		...
		methods: {
			...
			validNum(e) { //验证金钱
				this.$nextTick(() => {
					...
					// 	this.moneyMaxLeng = 8;
					// }
					var temp =/^(([1-9][0-9]*)|(([0]\.\d{1,2}|[1-9][0-9]*\.\d{1,2})))$/;// /^\d+\.?\d{0,2}$/;
					    if (temp.test(val)) {
					    } else {
					        val =val = val.replace(/[^\d.]/g, "");// val.match(temp, "");//val.substr(0, val.length - 1);
			...
			getBalance() {//获取房间余额
				...
				this.$http(
					...
					if (res.code === 200) {
						...
						that.roomInfo.accountBalance = res.data.balance||0
					}
				});
