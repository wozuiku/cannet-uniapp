...
<script module="echarts" lang="renderjs">
	import stecharts from '@/static/echarts.js'
	export default {
		data() {
			return {
				chart: null
		...
		methods: {
			...
			resizeChart(){
				var that=this;
				setTimeout(function(){//要加延时才可渲染完成
					//console.log("option",that.option);
					//console.log("that.chart",that.chart);
				//	that.chart&&that.chart.update(that.option);
					...
					// 			// 判断是否设置tooltip的位置
					// 			if (option.tooltip.positionStatus) {
					// 				option.tooltip.position = that.tooltipPosition()
					// 			}
					// 			// 判断是否格式化tooltip
					// 			if (option.tooltip.formatterStatus) {
					// 				option.tooltip.formatter = that.tooltipFormatter(option.tooltip.formatterUnit, option.tooltip.formatFloat2, option.tooltip.formatThousands)
					// 			}
					// 		}
					//     // 设置新的option
					// 	that.chart.setOption(option, option.notMerge)
					// 	}
